...
const WeightHistory = () => {
  ...
  const renderItem = ({ item, index }: any) => (
    <View
      className={`flex-row py-2 border-b border-gray-200 ${index === weightHistory.length - 1 ? 'bg-yellow-300' : index % 2 === 0 ? 'bg-gray-50' : ''} w-full`}
    >
  ...
  return (
    <SafeAreaView className="flex-1 bg-white px-4 pt-6">
      ...
      <View className="flex-row border-b-2 border-gray-300 pb-2 mt-6 w-full justify-between">
